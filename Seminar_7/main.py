# У вас есть код, который вы не можете менять (так часто бывает, когда код в глубине программы используется множество раз и
...
# имеет самую большую площадь. Напишите функцию find_farthest_orbit(list_of_orbits), которая среди списка 
# орбит планет найдет ту, по которой вращается самая далекая планета. Круговые орбиты не учитывайте: вы знаете,
...
#     # list_el = [i for i in list_of_orbits if i[0] != i[1]]
#     # list_ar = [(pi * i[0] * i[1]) for i in list_el]
#     # max_index = list_ar.index(max(list_ar))
#     # return list_el[max_index]
...
# 1. Исключить круговые орбиты
# 2. Найти площади
# 3. Найти максимальную площадь
# 4. Вывести соответствующий кортеж
